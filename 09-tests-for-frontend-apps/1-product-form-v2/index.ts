...
export default class ProductForm {
  ...
  private dragndropList: SortableList | undefined;
  ...
  private async loadProduct() {
    ...
    if (product && product[0]) {
      ...
      if (product[0].images) {
        this.dragndropList = this.createImageList(product[0].images)
        list.append(this.dragndropList.element);
      }
    }
  }
  ...
  private addImage(url: string, source: string) {
    ...
    this.dragndropList?.addItem(li);
  }

  private createImageList(images: { url: string; source: string }[]): SortableList {
  ...
  public async save(): Promise<void> {
    ...
    const response = await fetchJson<ProductSaveResponse>(url, {
      method: method,
      headers: {
